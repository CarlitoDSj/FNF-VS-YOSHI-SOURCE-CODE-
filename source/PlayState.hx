...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'flower-garden':
				dialogue = CoolUtil.coolTextFile(Paths.txt('flower-garden/Dialogue'));
			case 'underground':
				dialogue = CoolUtil.coolTextFile(Paths.txt('underground/Dialogue'));
			case 'spikey-stroll':
				dialogue = CoolUtil.coolTextFile(Paths.txt('spikey-stroll/Dialogue'));
		}
				...
				case 'island':
					{
							...
							var bgsky:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('stage/sky', 'yoshi'));
							...
							var backPillers:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('stage/back', 'yoshi'));
							...
							boshi.frames = Paths.getSparrowAtlas('stage/boshi', 'yoshi');
						...
						//	boshi.updateHitbox();
							...
							var middleStage:FlxSprite = new FlxSprite(-650, -50).loadGraphic(Paths.image('stage/middle', 'yoshi'));
							...
							var frontBushes:FlxSprite = new FlxSprite(-400, 800).loadGraphic(Paths.image('stage/front_bushes', 'yoshi'));
		...
		//if (SONG.song == "flower-graden") // for some reason game doesnt load and loads philly instead : /
			//{
				//generateSong("flower-graden");
			//}
		//else
			///{
	...
	function endSong():Void
	{
		...
		else
		{
			if (isStoryMode)
			{
				...
				{
					if (curSong.toLowerCase() == "spikey-stroll") // RENAME IT TO LAST SONG
						{
						//FlxG.sound.playMusic(Paths.music('freakyMenu'));
						//FlxG.switchState(new EndingState(health >= 1));
						FlxG.switchState(new VideoState('assets/videos/boshi/boshi.webm', new StoryMenuState()));
